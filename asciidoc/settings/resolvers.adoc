...
|link:../resolver/url.html[MirroredURL]|Standard|Finds Ivy files and artifacts in any repository accessible with URLs from a mirror list.
...
|link:../resolver/osgiagg.html[OSGi-agg]|Composite|Delegates the finding to a chain of sub resolvers supporting OSGi bundles.
|=======
